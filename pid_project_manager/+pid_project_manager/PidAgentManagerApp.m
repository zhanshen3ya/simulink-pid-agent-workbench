    ...
    methods (Access = private)
        ...
        function data = catalogTable(app)
            ...
            for index = 1:count
                ...
                data(index, :) = {item.selected, char(item.parentSystem), ...
                    ...
                    item.steadyStateErrorAbsMax, item.trackingRmseMax, ...
                    item.maxAbsCurrentMax, item.outputRippleMax, ...
                    item.controlSaturationFractionMax, evidence, char(item.status)};
            end
        end
        ...
        function syncCatalogFromTable(app)
            ...
            for index = 1:size(data, 1)
                ...
                app.Catalog.controllers(index).trackingRmseMax = double(data{index, 23});
                app.Catalog.controllers(index).maxAbsCurrentMax = double(data{index, 24});
                app.Catalog.controllers(index).outputRippleMax = double(data{index, 25});
                app.Catalog.controllers(index).controlSaturationFractionMax = double(data{index, 26});
            end
