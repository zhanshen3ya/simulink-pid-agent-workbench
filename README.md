...
当前发布为 `v0.5.0-beta.1` 预发布版本，功能和兼容性仍在完善。
...
级联双环需要将外环控制输出映射为内环参考信号，将外环设为主评价环，并使内环调节时间上限小于外环。两个 PID 不是级联关系时，应将二者都标记为“耦合环”，使用自动或联合调节策略。
...
只有通过全部硬指标的候选可以写入模型。写入前程序会确认历史任务的完整模型路径与当前已读取模型一致，保存模型备份和原始 PID 参数，并核对任务开始时记录的模型 SHA-256 指纹；模型路径不一致或模型在调参后发生变化时会拒绝应用。回滚前还会再次核对应用后的指纹，避免覆盖后来保存的模型修改。
...
Buck 示例分别记录外环 `r -> y`、控制量 `iRef`，以及内环 `iRef -> iL`、控制量 `u`。两组 PID 按内环、外环、联合微调三个阶段运行，并分别检查跟踪误差、调节时间、限幅和电流峰值。`r`、`y`、`iRef`、`iL`、`u` 都可在界面中人工确认。
...
- 最佳参数不会在无用户确认的情况下写回模型；只有已完成任务中通过全部硬指标的最终结果可以手动应用。
- 自动回滚仅适用于 PID Agent 最近一次写入，模型在写入后被再次修改时会停止自动回滚以保护新修改。
- Windows 以外平台尚未完成系统测试。
